...
class ModelEvaluation:
    ...
    def initiate_model_evaluation(self) -> ModelEvaluationArtifacts:
        ...
        try:
            ...
            all_losses_dict, all_losses = self.evaluate(
                trained_model, test_loader, device=DEVICE
            )
            os.makedirs(self.model_evaluation_config.EVALUATED_MODEL_DIR, exist_ok=True)
            all_losses_dict.to_csv(
                self.model_evaluation_config.EVALUATED_LOSS_CSV_PATH, index=False
            )

            s3_model = self.get_model_from_s3()
            s3_model = torch.load(s3_model, map_location=torch.device(DEVICE))

            s3_all_losses_dict, s3_all_losses = self.evaluate(
                s3_model, test_loader, device=DEVICE
            )
            ...
            if s3_all_losses > all_losses:
                ...
                model_evaluation_artifact = ModelEvaluationArtifacts(
                    is_model_accepted=is_model_accepted, all_losses=all_losses
                )
                ...
                model_evaluation_artifact = ModelEvaluationArtifacts(
                    is_model_accepted=is_model_accepted, all_losses=s3_all_losses
                )
